	...
	for(int i =0;i<size;i++)
	{
		for(int j=0;j<size;j++)
		{
			if(*(*(board+i)+j)!=0)
			{
				...
				{
					if(((k!=j)&&(element==*(*(board+i)+k)))||((k!=i)&&element==*(*(board+k)+j)))
					{
						return 0;
						exit(1);
					}
...
int main( int argc, char **argv ) {              
	...
	int **check_board;
        check_board = malloc(size * sizeof(int*));
        if(check_board==NULL)
	{
		printf("invalid");
 		exit(1);
	...
	for(int i =0; i< size; i++)
	{
		*(check_board + i) = malloc(size * sizeof(int));
	}
	...
	int return_value = valid_board(check_board,size);
        if(return_value == 0)
	{ 
		printf("invalid");
	}
